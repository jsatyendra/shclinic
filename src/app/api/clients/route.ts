...
export async function POST(request: Request) {
  try {
    ...
    const createClient = db.transaction(() => {
      ...
      db.prepare(`
        INSERT INTO clients (id, client_number, name, age, gender, height, weight, bloodPressure, bloodGlucose, address, phoneNumber, startDate, followUpDate, notes, status, isAcute)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        newId,
        clientNumber,
